    ...
    private func fetchWeatherData() {
        ...
        dataManager.weatherDataForLocation(latitude: latitude, longitude: longitude) { (response, error) in
            if let error = error {
                ...
                // Le pasamos la "response" que nos trae el día.
                self.dayViewController.viewModel = DayViewViewModel(weatherData: response)
                print("weatherData: response", response)
                // Configure Week View Controller, aquí pasa lo mismo que la anterior línea.
                // Le pasamos el array --> "let dailyData: [WeatherDayData]", ya que la response nos
                // trae el array con todos los días y accedemos a todos ellos con "response.dayliData"
                self.weekViewController.viewModel = WeekViewViewModel(weatherData: response.dailyData)
                print("weatherData: response.dayliData", response.dailyData)
                
                /* NOTA:  let weatherData: WeatherData --> propiedad que apunta al modelo de datos --> "WeatherData".
                 
                 struct WeatherData { -->  !!!! MODELO DE DATOS !!!!
                 
                 "self.dayViewController.viewModel = DayViewViewModel(weatherData: response)" --> ESTE MÉTODO COJE DE "response"
                 LAS PROPIEDADES A CONTINUACIÓN EXCEPTO --> "dailyData" QUE LA COJE --> WeekViewViewModel(weatherData: response.dailyData)
                 
                 EJEMPLO: response.time // response.lat... etc.
                 ...
                 let dailyData: [WeatherDayData] ---> ES LO QUE ESPERA --> elf.weekViewController.viewModel = WeekViewViewModel(weatherData: response.dailyData)
